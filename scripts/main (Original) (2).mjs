    ...
    try {
        ...
        if (WA?.prototype?.render) {
            ...
            function calcFeelsLikeF(tempF, windMph, humidity) {
                ...
                // Mild range — simple Steadman approximation
                // Accounts for light wind and moderate humidity
                const heatFactor = (humidity > 60 && tempF > 65)
                    ? (humidity - 60) * 0.02  // slight bump for muggy conditions
                    : 0;
                const windFactor = (windMph > 5 && tempF < 65)
                    ? -Math.min(windMph * 0.15, 5)  // light wind chill effect
                    : 0;

                return Math.round(tempF + heatFactor + windFactor);
            }
